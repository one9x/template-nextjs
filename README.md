# Next.js on One9x Pages

A minimal Next.js starter, wired to deploy to [One9x Pages](https://one9x.com)
on every push. **Use this template** to start from it, or read it as a reference
for an app you already have.

```sh
npm run build
one9x pages release ./out --site mysite --deploy
```
...
`output: 'export'` is a constraint on the app, not a deploy setting. API routes,
`getServerSideProps`, middleware, server actions and ISR do not exist in a
static build — the build fails naming them rather than shipping something
broken. If you need those, you need a server.

## Docs

- [Next.js on One9x Pages](https://one9x.com/docs/frameworks/nextjs) — the
